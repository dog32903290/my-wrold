...
Date: 2026-05-26 13:34 Asia/Taipei.
...
CANVAS1 read-only canvas surface (closed locally)
efce67d Add CANVAS1 read-only canvas surface
...
e493d82 Add COOK1 read-only cook plan
...
As of 2026-05-26 13:34 Asia/Taipei, APP1-APP7, WORK1-WORK5, ACTIVE1-ACTIVE3, PROJECT1-PROJECT4, OPEN1-OPEN3, SAVE1-SAVE3, STATUS1-STATUS3, the APP2 legacy proof artifact test cleanup, UI1, UI2, UI3, GRAPH1, GRAPH2, GRAPH3, CANVAS1, CANVAS2, CANVAS3, RUNTIME1, RUNTIME2, RUNTIME3, COOK1, and COOK2 are closed locally and not pushed. Current branch is `codex/cook2-cook-plan-proof-readback`; no active lane is selected after COOK2. Working tree was clean at lane start. Do not push unless explicitly requested.
...
COOK2 closed-lane files:
- docs/superpowers/plans/2026-05-24-native-canvas-master-progress.md
- docs/superpowers/specs/2026-05-26-cook2-cook-plan-proof-readback.md
- source/app/WorkbenchCookPlanSurfaceProofRunner.h
- source/app/WorkbenchCookPlanSurfaceProofRunner.cpp
- source/app/StartupProof.h
- source/app/StartupProof.cpp
- source/app/Main.cpp
- source/app/MainComponent.h
- source/app/MainComponent.cpp
- tests/WorkbenchCookPlanSurfaceProofRunnerTests.cpp
- tests/StartupProofTests.cpp
- CMakeLists.txt

COOK2 one-line proof before code:
...
./build/my_world_startup_proof_tests
MY_WORLD_PROJECT_DIR=/Users/chenbaiwei/Projects/my-world ./build/my-world_artefacts/我的世界.app/Contents/MacOS/我的世界 --dump-workbench-cook-plan-proof-and-exit
debug/workbench-cook-plan-proof/cook_plan_report.json written with ok true
ctest --test-dir build --output-on-failure -R "workbench_cook_plan_surface_proof_runner|startup_proof|workbench_cook_plan_surface|workbench_runtime_surface|workbench_session|workbench_app_controller"
...
STATUS2 closed-lane files:
- docs/superpowers/plans/2026-05-24-native-canvas-master-progress.md
- docs/superpowers/specs/2026-05-26-status2-app-status-proof-artifact.md
- source/app/AppStatusProofRunner.h
- source/app/AppStatusProofRunner.cpp
- source/app/StartupProof.h
- source/app/StartupProof.cpp
- source/app/Main.cpp
- source/app/MainComponent.h
- source/app/MainComponent.cpp
- tests/AppStatusProofRunnerTests.cpp
- tests/StartupProofTests.cpp
- CMakeLists.txt

STATUS2 one-line proof before code:

```text
App proof flag dumps app_status_report.json from WorkbenchAppController::appStatusSnapshot() after no-session, open, and save states.
```

STATUS2 verification target:
...
UI2 active-lane files:
- docs/superpowers/plans/2026-05-24-native-canvas-master-progress.md
- docs/superpowers/specs/2026-05-26-ui2-status-surface-proof-readback.md
- source/app/WorkbenchStatusSurfaceProofRunner.h
- source/app/WorkbenchStatusSurfaceProofRunner.cpp
- source/app/StartupProof.h
- source/app/StartupProof.cpp
- source/app/Main.cpp
- source/app/MainComponent.h
- source/app/MainComponent.cpp
- tests/WorkbenchStatusSurfaceProofRunnerTests.cpp
- tests/StartupProofTests.cpp
- CMakeLists.txt

UI2 one-line proof before code:

```text
The app proof flag dumps workbench_status_surface_report.json by running the same WorkbenchStatusSurface model that feeds the visible read-only labels.
```

UI2 verification target:
...
```text
--dump-workbench-status-surface-proof-and-exit writes debug/workbench-status-surface-proof/workbench_status_surface_report.json with ok true, headline save_work: save-ok commit-pending, rowCount 6, patch.ui2-main, active-work-opened, valid mapping 1/1, save-ok commit-pending, and the six surface row texts.
```
...
GRAPH1 closed-lane files:
- docs/superpowers/plans/2026-05-24-native-canvas-master-progress.md
- docs/superpowers/specs/2026-05-26-graph1-read-only-graph-surface.md
- source/app/WorkbenchSession.h
- source/app/WorkbenchSession.cpp
- source/app/WorkbenchGraphSurface.h
- source/app/WorkbenchGraphSurface.cpp
- source/app/MainComponent.h
- source/app/MainComponent.cpp
- tests/WorkbenchSessionTests.cpp
- tests/WorkbenchGraphSurfaceTests.cpp
- CMakeLists.txt

GRAPH1 one-line proof before code:

```text
The app reads the current workbench session's real graph summaries and displays them as a read-only graph surface.
```

GRAPH1 verification target:
...
CANVAS2 closed-lane files:
- docs/superpowers/plans/2026-05-24-native-canvas-master-progress.md
- docs/superpowers/specs/2026-05-26-canvas2-canvas-surface-proof-readback.md
- source/app/WorkbenchCanvasSurfaceProofRunner.h
- source/app/WorkbenchCanvasSurfaceProofRunner.cpp
- source/app/StartupProof.h
- source/app/StartupProof.cpp
- source/app/Main.cpp
- source/app/MainComponent.h
- source/app/MainComponent.cpp
- tests/WorkbenchCanvasSurfaceProofRunnerTests.cpp
- tests/StartupProofTests.cpp
- CMakeLists.txt

CANVAS2 one-line proof before code:

```text
The app proof flag dumps canvas_surface_report.json from the same WorkbenchCanvasSurface model used by MainComponent canvas labels.
```

CANVAS2 verification target:
...
CANVAS3 closed-lane files:
- docs/superpowers/plans/2026-05-24-native-canvas-master-progress.md
- docs/superpowers/specs/2026-05-26-canvas3-canvas-segment-closure.md

CANVAS3 one-line proof before closure:

```text
CANVAS1-CANVAS2 form a closed canvas segment from workbench graph positions to app proof readback artifact.
```

CANVAS3 verification target:
...
```text
None after COOK2 cook plan proof readback as of 2026-05-26 13:34 Asia/Taipei on `codex/cook2-cook-plan-proof-readback`. Not pushed. COOK3 cook segment closure is next if continuing.
```
...
Evidence:
- docs/superpowers/plans/2026-05-24-native-canvas-master-progress.md
- docs/superpowers/specs/2026-05-26-save2-app-save-proof-artifact.md
- CMakeLists.txt
- source/app/AppSaveProofRunner.*
- source/app/Main.*
- source/app/MainComponent.*
- source/app/StartupProof.*
- tests/AppSaveProofRunnerTests.cpp
- tests/StartupProofTests.cpp
...
Latest accepted result:
- Red test failed because `source/app/AppSaveProofRunner.cpp` did not exist after adding the SAVE2 test/CMake target.
- Startup proof red test failed because `StartupProofOptions::dumpAppSaveProof` and `StartupProofTaskId::appSave` did not exist.
- Follow-up red test failed until successful reports wrote empty `error`.
...
- Existing dirty file `tests/APP2WorkbenchOpenStatusProofRunnerTests.cpp` was left unstaged and untouched by SAVE2.
...
Latest accepted result:
- Red test failed before implementation because CreateActiveWorkProjectRequest/createActiveWorkProject did not exist.
- cmake --build build --target my_world_active_work_service_tests passed.
- ./build/my_world_active_work_service_tests passed.
- cmake --build build --target my-world passed, with the existing duplicate static-library linker warning.
- MY_WORLD_PROJECT_DIR=/Users/chenbaiwei/Projects/my-world ./build/my-world_artefacts/我的世界.app/Contents/MacOS/我的世界 --dump-app-workbench-session-proof-and-exit passed.
- Stable app proof still wrote both active_work_preparation_report.json and workbench_open_status_report.json.
