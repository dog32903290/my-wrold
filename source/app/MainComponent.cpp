      ...
      shouldQuitAfterStartupDump (startupProofOptions.quitAfterStartupDump)
{
    ...
    for (auto& label : workbenchRunLabels)
    {
        configureMeterLabel (label, "");
        addAndMakeVisible (label);
    }
...
void MainComponent::runStartupProofTask (StartupProofTaskId task)
{
    switch (task)
    {
        ...
        case StartupProofTaskId::workbenchRuntimeSurface:
            ...
            break;
        case StartupProofTaskId::workbenchCookPlan:
            dumpWorkbenchCookPlanProof();
            break;
        case StartupProofTaskId::workbenchRun:
            dumpWorkbenchRunProof();
            break;
    }
}
...
void MainComponent::resized()
{
    ...
    auto runRow = area.removeFromTop (24);
    workbenchRunLabels[0].setBounds (runRow.removeFromLeft (230));
    workbenchRunLabels[1].setBounds (runRow.removeFromLeft (140));
    workbenchRunLabels[2].setBounds (runRow.removeFromLeft (170));
    workbenchRunLabels[3].setBounds (runRow.removeFromLeft (180));
    workbenchRunLabels[4].setBounds (runRow.removeFromLeft (170));
    workbenchRunLabels[5].setBounds (runRow);
...
void MainComponent::dumpWorkbenchCookPlanProof()
{
    const auto directory = proofDumpDirectory (workbenchCookPlanProofDirectoryName());

    WorkbenchCookPlanSurfaceProofRunRequest request;
    request.outputDirectory = directory.getFullPathName().toStdString();
    request.candidateRoots = proofCandidateRoots();

    const auto result = runWorkbenchCookPlanSurfaceProof (request);
    if (! result.ok && ! result.statusText.empty())
    {
        finishProofDump (juce::String (workbenchCookPlanProofDisplayName()),
                         "failed",
                         result.statusText,
                         directory);
        return;
...
void MainComponent::dumpWorkbenchRunProof()
{
    const auto directory = proofDumpDirectory (workbenchRunProofDirectoryName());

    WorkbenchRunProofRunRequest request;
    request.outputDirectory = directory.getFullPathName().toStdString();
    request.candidateRoots = proofCandidateRoots();

    const auto result = runWorkbenchRunProof (request);
    if (! result.ok && ! result.statusText.empty())
    {
        finishProofDump (juce::String (workbenchRunProofDisplayName()),
                         "failed",
                         result.statusText,
                         directory);
        return;
...
void MainComponent::updateWorkbenchRunSurface()
{
    const auto surface = makeWorkbenchRunSurface (workbenchController.currentSession());

    for (std::size_t index = 0; index < workbenchRunLabels.size(); ++index)
    {
        auto& label = workbenchRunLabels[index];
